...
export default function Dashboard() {
  // FIXED: Use currentUser instead of user, and remove old auth methods
  const { currentUser } = useAuth()
	console.log('Dashboard - currentUser:', currentUser);
  	console.log('Dashboard - is logged in:', !!currentUser);
  ...
  // FIXED: Temporary localStorage methods until we implement Firebase portfolio storage
  const getPortfolios = () => {
    try {
      const portfoliosKey = `portfolios_${currentUser?.uid || 'guest'}`
      const stored = localStorage.getItem(portfoliosKey)
  ...
  const savePortfolios = (portfoliosData) => {
    try {
      const portfoliosKey = `portfolios_${currentUser?.uid || 'guest'}`
      localStorage.setItem(portfoliosKey, JSON.stringify(portfoliosData))
      return true
    } catch (error) {
  ...
  const deletePortfolio = (portfolioId) => {
    try {
      const current = getPortfolios()
      const updated = current.filter(p => p.id !== portfolioId)
      return savePortfolios(updated)
    } catch (error) {
  ...
  const updatePortfolio = (portfolioId, actualDataUpdate) => {
    try {
      const current = getPortfolios()
      const updated = current.map(p => 
        p.id === portfolioId 
          ? { ...p, actualData: actualDataUpdate }
          : p
      )
      return savePortfolios(updated)
    } catch (error) {
  ...
  // FIXED: Check currentUser instead of user
  if (!currentUser) {
    return (
  ...
  // FIXED: Get user name from Firebase currentUser
  const getUserName = () => {
